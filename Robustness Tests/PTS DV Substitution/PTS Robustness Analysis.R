...
## select the DNR Data PTS Included file from 
## GitHub repo/zip folder
data <- read_csv(file.choose())

## ordered logit with amnesty PTS
olrA <- polr(as.factor(politterr_a_PTS) ~ police + gdp_WDI_log10 + cameo_protests + trade_WDI + apts_lag + polity2_P4 + pop_WDI_log10 + lji_LS, data = data, method = "logistic")
summary(olrA)
...
clmA <-
  clm(
    ...
    nominal = ~ cameo_protests + trade_WDI + apts_lag + pop_mc + lji_LS,
    data = data
  )
summary(clmA)
...
clmS <-
  clm(
    as.factor(politterr_s_PTS) ~ police + spts_lag + polity2_P4 + lji_LS,
    nominal = ~ gdp_mc + cameo_protests + trade_WDI + pop_mc,
    data = data
  )
summary(clmS)
